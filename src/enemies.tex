...
\begin{itemize}
	...
	\enemSpecs{64 ($\sim$0.15 seconds of Iframe, Boomerang Cutter, Sting Chameleon and Rolling Shields hit for 8 damage) }{4 (contact), 2 (snakes)}{Angler-type mechaniloids that work to cleaning the seabed floor, with a motion sensor attached to its "lantern" part.}
	\begin{figure}[htp]
		\centering
	...
	\enemSpecs{32 (no Iframes)}{2 (contact), 2 (missiles), 3 (mines)}{A mechaniloid that is a smaller version of Volt Kurageil. It is equipped with depth charges and homing missiles, but its tentacles can't seem to move together. Its low stamina makes it rather weak.}
		\begin{figure}[htp]
		\centering
	...
	\enemSpecs{32 in all forms ($\sim$0.5 seconds of Iframe, Speed Burner deal 3-6 damages)}{2 (hand cannon), 3 (scatter shots), 4 (contact), 2 (shield)}{Extra-large sized private intruder (Raider) repulsion mechaniloid. When it searches an enemy with its radar, it scans the enemy's conduct patterns as well. When the radar discovers an enemy, Raider Killer's form is strengthened with a blue energy, and it enters a violent rage.}
	\begin{figure}[htp]
		\centering
	...
	\enemSpecs{18 ($\sim$0.97 seconds worth of invincibility frames)}{3 (contact)}{A substantiation of the Sigma Virus. It which moves and attacks at random from 
	3 patterns. Its name comes from "shikein" and "shuriken" (throwing star). The flying mechaniloid which generates it is called Genjibo.}
	\begin{figure}[htp]%
		\centering
	...
	\enemSpecs{48 ($\sim$0.116 seconds of Iframes, Storm Tornado hits for 9-10 damage)}{5 (contact), 4 (thunders)}{Thunder Slimer was born from a single question: \textit{``How large can a single cell become?''} This monster was born from said experiment. Its body is over three times as large as X, but may require approximately 10 more years before it reaches full growth. It has settled in the power plant, where it absorbs electricity and uses it to perform electric attacks against X.}
	\begin{figure}[htp]
		\centering
...
\begin{itemize}
	...
	\item[{\includegraphics[height=20px]{figures/X3/Enemies/Atareeter.png}}] \hypertarget{enem:Atareeter}{\textbf{Atareeter}}:
	...
	\item[{\includegraphics[height=20px]{figures/X3/Enemies/bladeysprite.png}}] \hypertarget{enem:Blady}{\textbf{Blady}}:
	...
	\enemSpecs{14, weak to Silk Shot, Spin Wheel and Magnet Mine}{2 (contact), 4 (cannon)}{2-footed walker type interceptor tank. Powerful mechaniloid that fires using two 200 mm cannons and enemy-seeking pursuit missiles.}
	
	
	
	\item[{\includegraphics[height=20px]{figures/X3/Enemies/carryarmsprite.png}}] \hypertarget{enem:Carry_Arm}{\textbf{Carry Arm}}: 
	...
	\item[{\includegraphics[height=20px]{figures/X3/Enemies/caterkillersprite.png}}] \hypertarget{enem:Caterkiller}{\textbf{Caterkiller}}:
	\enemSpecs{2, weak to Parasitic Bomb and Gravity Well}{2 (contact-normal), 3(contact-charged), 3(electric spheres), 8(charged electric spheres)}{Caterpillar-type mechaniloid which can move alongside walls. The range of its lightning shot is short, but it always returns to its body.}
	
	\item[{\includegraphics[width=30px]{figures/X3/Enemies/Crablaster.png}}] \hypertarget{enem:Crablaster}{\textbf{Crablaster}}:
	...
	\enemSpecs{6, weak to Parasitic Bomb and Spinning Blade}{1 (projectiles), 6(contact)}{\hyperlink{enem:Head_Gunner}{Head Gunner} with cannons attached, as well as stronger armor. Made from the parts produced in the weapons factory stage.}
	...
	\enemSpecs{1, weak to Sonic Slicer, Speed Burner and Bubble Splash}{4 (lightning)}{Hatched from the capsule weapon number one dropped by Sky Farmers. They fly and attach to the enemy, then attack by calling upon thunder.}
	...
	\enemSpecs{22, weak to Strike Chain and Crystal Hunter}{-}{Device which creates and maintains the artificial weather inside the Weather Control Center. Because it is such a precision instrument, it has a fault which makes it easily affected by outside stimulus.}
