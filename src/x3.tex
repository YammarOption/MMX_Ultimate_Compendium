...
As the third entry in the series, Mega Man X3 borrows many features from its predecessors, while also adding new features to keep the gameplay fresh:
\begin{itemize}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/A_burst.jpg} Acid Burst}{Acid Burst}}\label{Acid_Burst}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/P_bomb.jpg} Parasitic Bomb}{Parasitic Bomb}}\label{Parasitic_Bomb}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/T_thunder.jpg} Triad Thunder}{Triad Thunder}}\label{Triad_Thunder}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/S_blade.jpg} Spinning Blade}{Spinning Blade}}\label{Spinning_Blade}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/R_splasher.jpg} Ray Splasher}{Ray Splasher}}\label{Ray_Splasher}
...
\end{figure}
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/G_well.jpg} Gravity Well}{Gravity Well}}\label{Gravity_Well}
...
\end{itemize}
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/F_shield.jpg} Frost Shield}{Frost Shield}}\label{Frost_Shield}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X3/weapons/T_fang.jpg} Tornado Fang}{Tornado Fang}}\label{Tornado_Fang}
...
\begin{figure}[htp]
	\centering
	\includegraphics[height=\portraitsize]{figures/X3/X3_Armor.png}
	\caption{The Third Armor.}
\end{figure}
...
\begin{itemize}
	...
	\item \emph{Head Parts}: Found in Tunnel Rhino's stage, in a path opened by a boulder destroyed after using the charged Triad Thunder. This parts will add a radar to X's equipment which will map the current stage's layout and point out yet-to-discover collectibles, such as other armor capsule, life-up and sub-tanks. The map is displayed only once as soon as X enters a stage, but it requires an input for the player to close, so it can remain on screen for as much as the player wants. Additionally, on the stage selection screen additional information for each stage will be displayed, showing collectibles for each stage and highlighting missed ones.
...
Once a benevolent soul driven by a passion for crafting ice sculptures~\cite{Xcoll1:Manual_X3}, Blizzard Buffalo, also known as the \textit{Silver Snowman}~\cite{book:MMX_Complete_art}, was a reploid designed to operate in cold weather as a safety guard at the ski resort where he was employed~\cite{wayback:X3_resources,wiki:Blizzard_buffalo}. However, at a certain point, he relocated to Doppler Town, where he became entangled in Doppler's scheme and succumbed to corruption by the Sigma Virus. This transformation made him highly aggressive, resulting in the freezing of a portion of the city. Eventually, he was mercifully put down by the hand of X.


Facing Blizzard Buffalo in battle is not particularly challenging. The primary difficulty arises from the long arena which can hide Buffalo's attacks if executed from off-screen, catching the player off guard. Blizzard Buffalo employs only three main attacks: \emph{Charge}, \emph{Ice Bullet}, and \emph{Freeze Beam}. During his charge attack, Buffalo launch himself toward X and, upon contact, he traps X between his horns and slam him into the wall. This attack can be easily avoided, as X can outpace Buffalo by dashing and utilize the walls for wall-jumping above him. In his second attack, Buffalo throws three snowballs at X, which, upon impact with a surface, transform into ice spikes. X can easily avoid the move by keep moving, and the spikes can be neutralized by firing at them. Finally, when Buffalo is low on health, he resorts to his Freeze Beam, a large beam that covers the entirety of the arena in length and a good portion in height, capable of immobilizing X. However, this attack inflicts no damage to X himself, but rather exposes to a subsequent attack from the boss.
\begin{figure}[htp]
	\centering
	\begin{subfigure}{\linewidth}
		\centering
...
The second segment of the stage is set outside the carrier, with nothing particularly noteworthy aside from some enemies. This portion ends with X re-entering the colossal airship via an ascending elevator, with enemies positioned to attack X from both sides.

Lastly, the concluding part of the stage takes place within what can be depicted as an ammunition storage area. Immediately following the elevator, players will find a Ride Armor platform, facilitating travel in the subsequent segment. Here, the floor primarily consists of what can be assumed to be giant shells, which fall as soon as X steps on them, requiring quick movements to avoid falling. Beyond this area, X arrives at a wall seemingly impeding the armor's progress. However, with precise platforming, it is possible to climb up while retaining it. From this vantage point, only a narrow corridor and another ascent (this time impossible for the armor) separate X from the boss door.
...
Gravity Beetle relies on three primary attacks. The first, known as the \emph{Gravity Well} (or Bug Hole in Japanese~\cite{wiki:Gravity_beetle}), involves Beetle launching two black holes—one towards X and another above him. These projectiles bounce off walls, increasing in size with each hit, up to a maximum of three times. Additionally, when Beetle's health is low, he employs an enhanced version of this weapon, summoning a large stationary black hole at the center of the arena. This obstacle increase his jumping capability and represents a threat to X upon contact. Beetle also possesses a \emph{Charge Attack}: after becoming invulnerable, he charges towards X in an attempt to strike him. If successful, X is launched upwards in a way similarly to Boomer Kuwanger's Death Lift.
...
The subsequent section of the stage is entirely submerged. Right at the beginning, players are presented with a Robot Ride Platform, allowing for the summoning of a Ride Armor. It is worth noting, however, that any armor aside from the Frog Change will get damaged and become unusable once submerged. This section offers two primary routes: the upper path is relatively less dangerous, though it ends with fans that push X backward if he lacks the armor, making impossible to proceed. Contrarily, the lower path is more hazardous, filled with gaps and spikes along the ceiling, but it is traversable to the end. Both paths eventually converge, leading to a door that grants access to the \hyperlink{miniboss:Hotareeca}{Hotareeca} miniboss, which employs homing missiles and deploying mines as its sole modes of attack.

Beyond this segment, the final one begins. Here, X must climb up to reach the outside, traverse the dam's upper section while avoiding gaps, and descend into the subsequent structure, where the boss awaits.
...
\subsection{Heart Tank}
The Heart Tank is found at the very beginning of the stage. During the climb with water flowing the opposite direction, if players keep climbing above the door leading to Bit/Byte, they will reach an upper platform with the Heart Tank in plain sight.
...
The pink capsule hiding the Foot Chip is found in a position which mirrors the Change K. X has, in fact, to reach the water's surface and go right to reach an entrance in the wall leading to the capsule.
\begin{figure}[htp]
	\centering
...
Toxic Seahorse was a clandestinely developed reploid, distinguished by the unique feature of possessing a body composed of a liquid metal alloy. This attribute provided him the ability to manipulate his form and seamlessly blend into his surroundings~\cite{wiki:Toxic_seahorse,wayback:X3_resources}. Acting on Doppler's directives, Seahorse assumed control of the Monarch Dam, the largest dam in Doppler Town, with the intent of cutting the city's water supply.

In battle, Toxic Seahorse follows a fairly predictable pattern. He frequently umps  back-and-forth, occasionally alternating these movements with one of three attacks. His initial move, the \emph{Acid Burst}, sees Seahorse fire an acid projectile towards X, which then bounces upward towards the ceiling. X can eliminate the projectile by firing at it, and with each bounce, it releases two droplets of acid. The second attack at Seahorse's disposal is his \emph{liquefy} ability, which makes him invincible as he blends with the ground, only to quickly reappear beneath X moments later. Finally, when Seahorse health falls below half, Seahorse deploys the \emph{Bouncing Acid Ball}. This move closely resembles the Acid Burst, but Seahorse releases two acid projectiles that bounce along the ground in pursuit of the player, and cannot be destroyed.
...
\end{figure}
There are no perfect strategies to face Toxic Seahorse without risks. The main dangers from the battle arise mainly from the boss constantly moving around the arena, keeping pressure on players, alongside the acid balls which, even if avoided, cans still deal damages simply thanks to the droplets that scatter as they bounce. Removing them as soon as Seahorse fires them is a viable option, but they require more multiple shots to break, which gives Seahorse enough time to move around. In the end, preparation is crucial for this battle, as it can easily degenerate in a test of endurance between who has more health points between the boss and X. Using the boss' weakness, the Frost Shield, can provide some help, as the weapon deals increased damage to the boss, despite te difficulty which can arise in firing it with the correct timing.


While engaging Seahorse conventionally with the X-buster or even employing his vulnerability may present a degree of challenge to players, it is also possible to exploit a glitch in Seahorse's invincibility frame system, potentially leading to the boss being defeated in just two AI loops. The details on this glitch are listed in section~\ref{glitch:Sehorse}.
...
If the Airborne Aircraft Carrier was the second largest stage in the game, the Power Control Center seizes the title of the largest. This, combined with the abundance of traps, some more lethal than others, results in a stage that continuously tries to chips away players' health, potentially leading in their defeat or, at the very least, making the final boss encounter, represented by Volt Catfish, more challenging.

The stage is structured in a ``upside-down U'' configuration, forcing X to initially climb up and reach the pinnacle before subsequently descending once again. As such, the initial segment of the stage consists in a series of climbs, involving both wall climbing and the use of an elevator flanked by spiked walls. Throughout this section, X encounters stationary adversaries in the form of \hyperlink{enem:Meta_Capsule}{Meta Capsules}, \hyperlink{enem:Crablaster}{Crablasters}, and \hyperlink{enem:Trapper}{Trappers}, foes which are strategically positioned to hinder X's progress and damage him. The latter, in particular, can be especially annoying, given their invulnerability and quick fire when X's entry into their sensor's range.

Following the conclusion of the initial segment, which ends with a brief passage situated outside the plant, the descent into the second part of the stage commences. Alongside the aesthetic  shift, new hazards materialize in the form of electrified bars affixed to the walls, which deals contact damage if X touches them. Apart from this, the roster of enemies remains consistent, requiring players to be careful while navigating to reach the lowermost point and gain access to the boss chamber.
...
\subsection{Heart Tank}
The heart tank in this stage is hidden at the end of a side passage accessible during the second elevator ascent. To reach it, players must avoid to immediately exit the elevator when the path becomes available and instead remain inside, allowing it to continue its upward journey up to the top, where is a second exit, near a spiked ceiling waiting for the players. Upon arrival, players will note the heart tank positioned on a bed of spikes, requiring precise wall jumps to retrieve it.
...
Once characterized by kindness and a gentle behavior, Volt Catfish was built with a powerful high-voltage generator integrated into his body. This unique feature effectively made him a mobile power plant, capable of providing electric energy support to an entire city during times of crisis~\cite{wayback:X3_resources}. This distinction led to him being dubbed the \textit{``Rescue Power Plant''}. Tragically, during the incident in Doppler Town, Catfish fell victim to the Sigma Virus, which eradicated all traces of his original personality. Transformed into a Maverick, Catfish seized the city's power plant, potentially with the intent of redirecting its electricity to Doppler's lab~\cite{wiki:Volt_catfish}. However, the overwhelming surge of power he absorbed drove him into a berserk state, making him to challenge anyone who dared to confront him~\cite{Xcoll1:Manual_X3}. X accepted this challenge, ultimately bringing an end to Catfish's struggle.
...
Depending on the player's familiarity with Crush Crawfish's boss fight, the encounter can range from challenging to relatively straightforward. Engaging this boss as intended can be  challenging to most players, requiring precise movements and quick reflexes to avoid falling victims of Crawfish claws, that can quickly decimate players' health, thus leaving very few space for errors. It does not help also the fact that Crawfish's main weakness resides in the Triad Thunder, a weapon that requires either precise positioning or close proximity to land a successful hit, without providing any additional benefits aside from increased damage output. On the contrary, seasoned players have the potential to completely exploit this battle by capitalizing on not one, but two key glitches that affect Crawfish. The first glitch has origin in Crawfish's faulted AI  and is described in section~\ref{glitch:Crawfish_AI}. This glitch allows players to safely stay in the boss room to plan the next move, without worrying of Crawfish's actions. The second, and most powerful glitch, is described in section~\ref{glitch:Crawfish}, and allows instead to instantly deplete the boss's health with a single release of the Triad Thunder. Given these circumstances, by combining these exploits it is possible to immediately end the boss fight. The strategy involves first hiding X in the corner immediately at the fight's commencement. Then, while the boss roams, the player releases a Triad Thunder and begin climbing the wall, simultaneously charging up another Triad Thunder. If executed correctly, Crawfish will be struck by the diagonal bolt, while the charged version will be ready. At this point, X merely needs to descend and unleash the shot, bringing the battle to an end. It is possible that, while executing this technique, Crawfish may release a boomerang in pursuit of X before succumbing to the attack. However, this is inconsequential, as such obstacles do not interrupt the charging process and may even prove advantageous in speeding up the descent.
...
\section{Quarry}
The Quarry Stage sees X traveling in the depth of a mine to reach Tunnel Rhino, who waits in the deepest part of the area.

This stage stands out as one of the most challenging to navigate, primarily due to the many threats it presents. In the initial segment, the primary danger arises from the platforming challenges over spiked pits required to progress. Here, as an additional challenge, machines will periodically drop mud , hindering X's movements and potentially dragging him into the lethal spikes if caught.

Moving past this first section, the dangers shifts as the climbing becomes more dangerous and with less platforming required, with mud machines replaced by falling boulders that threat to crush X. Towards the end of this section, the \hyperlink{miniboss:Hell_Crusher}{Hell Crusher} appears as the sub-boss for this area. This formidable enemy is a significant challenge for players due to its ability for deal heavy damage, despite having only two distinct attacks. Typically, the Hell Crusher charges towards X in an attempt to strike him with its spike, a move which can be easily avoided by wall jumping. However, it's worth noting that the impact can briefly stun X if he was climbing, potentially causing him to fall down onto the spike itself. Alternatively, if X stall on the wall for an too long, the Hell Crusher will lift its right arm, grapple itself to the roof, separating his torso to the lower body in the process, and use the spare arm to attack X. While it may initially seem opportune to jump between the two segments and attack from behind, the spikes on the rear still are a threat to the player, making this move counterproductive. Importantly, the Hell Crusher only takes damage when hit on its torso, making it difficult to land a blow while avoiding its attacks. However, the absence of invincibility frames makes a charged Triad Thunder the most effective means to dispose of the sub-boss, eliminating it with a single shot.

Even after overcoming the Hell Crusher, the challenge remains. To access the boss lair, players must navigate a final section filled with enemies seeking to finish X after the preceding battle. Dying to these enemies forces a rematch against the Hell Crusher, underlining the importance of finishing its fight with a enough health left. If players successfully navigate this last stretch, they will arrive at the boss door leading to Tunnel Rhino.
\begin{figure}[htp]
	\centering
...
\end{figure}
The capsule containing the Head Parts can be obtained in the same way as the Heart Tank was found. During the climb with the falling rocks, players will notice a second claw holding a boulder over another spiked pit. By repeating the process done with the Heart Tank, players will reach an opening to the surface, where the Capsule can be found.
...
Despite his formidable appearance, Tunnel Rhino doesn't rely solely on strength in battle, as he fight using a combination of melee and ranged attacks. Rhino's main attack is his \emph{Charge 1}, a high-speed dash towards X, with the potential of causing the surroundings to shake and knocking X down if he was on a wall. When on low health, Rhino will also rely on the \emph{Charge 2} variant, becoming invincible and executing a faster dash dealing more damage. Another attack players have to watch for is his \emph{Tornado Fang}, where Rhino shoots the three drill he possesses in the direction they are facing: one forward and two diagonally. 
\begin{figure}[htp]
	\centering
	\begin{subfigure}{\linewidth}
		\centering
...
Shortly after the starting point, players will reach a spacious room with an exit above the entrance. Typically, one would need to reach the opposite side, climb, and return on the upper path to exit. However, if players have acquired the foot parts, they can skip this process by using the upward dash to reach the wall above the entrance and proceed from there. Beyond the large room, the path becomes safer, featuring fewer enemies and less spikes. This section ends in a long ladder leading down to the first boss door, where X faces \hyperlink{miniboss:Worm_seeker-r}{Worm Seeker-R}, a centipede-like mechaniloid. This miniboss moves slowly in the arena, descending vertically from the top corners, ascending vertically from the middle to the roof, or arcing from one corner to the other. While moving, the boss will also perform its only attack, by dropping two mines that bounce at various heights and explode after some time. The fight can be considered ''challenging'' only due to the miniboss's single weak spot, the head, the brief window players have to hit it, and its substantial amount of HP. In this scenario, the most effective strategy is to use powerful weapons such as Triad Thunder and Frost Shield~\cite{wiki:Worm_seeker}.

After defeating the sub-boss, the stage returns to a setup similar to the first section, only with more enemies and pits. To balance the increased difficulty, players can summon a Ride Armor from the start, facilitating travel through this portion.

The final section begins after the second set of doors, followed by a ladder leading X deeper into the stage. Here, \hyperlink{enem:Atareeter}{Atareeters} start digging the floor, transforming it into a spiked path. Although this path doesn't instantly kill X, it can becomes challenging to traverse it if these enemies are left unchecked. Fortunately, this last section is relatively short and concludes with the boss door leading to Neon Tiger.
...
\subsection{Heart Tank}
This last collectible is hidden immediately before the door leading to the boss, on top of the wall where the door is. Here, again, using a Tombort can be helpful to avoid erroneously hitting the door, triggering the boss fight.
\begin{figure}[htp]
	\centering
...
Unlike other reploids, Neon Tiger was specifically created to combat humans even before turning Maverick. Initially deployed in a natural reserve~\cite{wiki:Neon_tiger}, he served as a poacher hunter, safeguarding the remaining animals from heartless hunters. This role earned him the title of ``\textit{Protector of the Jungle}''\cite{book:MMX_Complete_art} or alternatively, ``\textit{Guardian God of the Jungle}''\cite{wayback:X3_resources}. Neon Tiger was optimized for prolonged operation, using solar energy as his primary power source and using his advanced visual sensors to quickly strike from the darkness with his sharp claws~\cite{wayback:X3_resources,Xcoll1:Manual_X3}. During the incident at Doppler Town, Tiger became infected with the Sigma virus, turning Maverick, but  no detailed information are given about the specific changes he underwent during this transformation. Ultimately, X stopped him after a fierce duel in the park he once protected.
\begin{figure}[htp]
	\centering
	\begin{subfigure}{.4\linewidth}
		\centering
...
Neon Tiger's fighting style revolves around fast and powerful strikes, grouped in two distinct attack modes: ranged attacks and melee strikes. For ranged attacks, Tiger employs the Ray Splasher, where he shoots a volley of beams from his tail either from the ground (\emph{Ray Splasher 1}) or while clinging to a wall (\emph{Ray Splasher 2}). Typically, after a ranged attack, Tiger switch to his melee move, the \emph{Dive Attack}. This move sees Tiger leaping off the wall near X and attacking him with his claws. Additionally, when at low health, Tiger performs his second melee move, the \emph{Rush Attack}. With this attack, Tiger charges himself in an invincible state, dashes towards X, and hits him with his claws; after that, whether the previous attacks missed or not, Tiger leaps into the air, covering the scenario where X was climbing the wall to avoid the move.
\begin{figure}[htp]
	\ContinuedFloat
	\centering
	\begin{subfigure}{.4\linewidth}
...
The stage is divided in two sections. In the first part, X must traverse enemies to reach the boss and defeat him. During this path, players will encounter a climbing section with falling rocks and an elevator. If Volt Catfish has been defeated, the elevator will be operative, providing a safe climb towards the boss. Once the boss (see \ref{boss:Vile2} for strategies) has been defeated, the second part of the stage begins. A 50-second timer starts running as Vile has mined the factory. Because of this, players must quickly travel to the exit to avoid imminent death. Immediately after the boss room, players are provided with a significant number of health pickups and even a life-up, to heal X after the fight and give a second chance should the escape fail. The escape sequence itself is straightforward, starting with a long corridor filled with enemies, followed by a second elevator behaving exactly like the first one. The sequence concludes with a final run over collapsing bridges, to reach the escape capsule.
\begin{figure}[htp]
	\ContinuedFloat
	\centering
	\begin{subfigure}{\linewidth}
...
\end{figure}
Once escaped, the stage becomes unavailable, returning players to the stage they initially entered from.
...
After defeating the last Maverick, Dr.~Cain contacts X to inform him that Doppler's secret lab has been located by analyzing the remnants of defeated Mavericks. Subsequently, Doppler's fortress appears on the screen, prominently situated in the middle of Doppler Town, alongside a new entry point for the game's final stages. As in previous titles, players must be mindful that their progress will not be saved through the password system, requiring to complete the upcoming four stages in a single session.

The first of Doppler's stages takes place in what resembles a disposal center~\cite{wiki:Doppler_stage} and a prison where Doppler modifies Reploids into combat machines\footnote{\textit{``X, there is an army of powerful Mavericks in Dr. Doppler’s lab! They’ve been modified by Dr. Doppler into war machines. Be careful!'' - Zero, Doppler Stage A}}. Consequently, the stage is filled with powerful enemies and deadly hazards, making it one of the most challenging levels to navigate. However, it is also the stage where the secret capsule can be acquired

The initial segment of the stage is arguably the most dangerous, extending from the beginning to the stage's sub-boss. Right from the start, X faces the challenge of platforming with a spiked ceiling that can fall down to intercept and eliminate him while jumping. To navigate this, players are suggested not to immediately attempt a jump but to test the ground with smaller jumps to trigger the trap first and then proceed with the jump. Additionally, using powerful weapons like the Spinning Blade or Triad Thunder to quickly eliminate enemies is recommended. Advancing in the stage, players encounter another hazardous trap: the classic climb with closing walls. However, in this iteration, the walls close in alternating directions, making it impossible to stick to one side and climb from there.
...
In battle, Bit employs three distinct moves. His primary attack, the \emph{Charge}, involves Bit leaping into the air and dashing forward with his beam saber as soon as he aligns with X. While dashing, the sword acts as a form of protection, nullifying frontal attacks. Bit's second attack is his \emph{Ring}, an energy ring thrown in front of him, which drops on the floor and moves back and forth for a while before disappearing. Although this attack does not inflict damage, it can temporarily block X's movement until he receives a hit or the player rapidly mashes buttons to break free, with the risk of being hit by a follow-up in the meantime. Bit's final attack is the \emph{Homing Shot}, an energy sphere he throws into the arena, chasing X for a while or it connects with X twice. When Bit is low on health, he increases the number of projectiles he shoots, transitioning from one ring and homing shot to two each every time.
\begin{figure}[htp]
	\centering
	\begin{subfigure}{.4\linewidth}
		\centering
...
The best way to deal with Bit is to understand how his \emph{charge} attack works. By exploiting the fact that Bit start the dash only when aligned with X, players can force him to execute the move while in mid-air by using small jumps as a bait. This strategy gives sufficient room to dodge the move by passing under the boss and allows players to hit him as the move ends. Also, it's crucial to keep track of the rings and shots Bit fires, as they can disrupt players' movement and quickly accumulate damage on X. Additionally, it's important to note Bit's weaknesses, which are the Frost Shield and Triad Thunder. These weapons not only deal increased damage to him but are also capable of permanently destroying Bit if the final hit is delivered by one of these weapons. Permanently defeating Bit is one of the two steps to alter the final boss encountered in Doppler Stage A
...
\end{figure}
Byte, known as Mandarela BB in Japan, serves as the second member of the Nightmare Police, Doppler's personal agents. Like his comrade Bit, Byte was constructed by Doppler using specifications from the best reploids of the time~\cite{book:MH_field_guide}, and he is incredibly loyal to his creator. However, while Bit was designed to maintain a balance between defense, speed, and offense, Byte was built for sheer strength, with a raw power potentially rivaling Sigma himself. During the events in Doppler Town, Byte, alongside his comrade, was dispatched to pursue X, resulting in a clash between the two. In contrast to Bit, Byte faced X with great respect~\cite{wiki:Byte}, believing their fight was destined\footnote{\textit{``My name is Mandarela. I shall fight according to fate''}-Byte~\cite{wordpress:X3_japanese_script}.} Depending on the outcome of the battle, Byte may either retreat after getting  injured and promise a rematch, or perish, congratulating X and bidding him farewell.
\begin{figure}[htp]
	\centering
	\begin{subfigure}{.4\linewidth}
		\centering
...
Despite his size and incredible power, Byte employs only two attacks in his fight, one of which doesn't even deal damage. The first move Byte almost guarantees to use is his \emph{Gravity Shot}, a gravitational mine he shoots that attaches to the opposite wall and which acts as a magnet, repelling X and pushing him toward the boss. Following this move, Byte employs his \emph{Charge}, a 4-hit combo involving a shoulder tackle towards X that sends him against the ceiling, only to intercept him midair with a punch that knocks X into the wall. The combo initiates only if the shoulder attack connects with X; otherwise, Byte will simply ram into the wall and destroy any mines present.


Despite the limited attack pool, Byte represents a formidable challenge due to the immense damage output he can inflict. Moreover, the gravitational mine hinders X's movements, especially when it comes to wall jumping, making performing them more difficult. The key to win the avoid such problem is for to use the magnet pull to X's advantage by dashing towards the wall and immediately dash-jumping off it, utilizing the magnet pull to increase the distance and pass above Byte. This move allows players to dodge Byte's attacks and provides sufficient time to answer with their own. In particular, Byte suffers heavy damage from the Tornado Fang and the Ray Splasher, either of which will permanently eliminate him if they deliver the final blow.
...
\end{figure}
The final boss encountered at the end of Doppler Stage A depends on the outcome of the previous battles against Bit and Byte. If, for any reason, either one of the two members of the Nightmare Police survives the clash with X, the survivor(s) will reappear in the boss room. On these occasions, they inform X that upon their defeat, Doppler has modified their bodies to increase their power. Additionally, if one of the two was killed, the surviving one will commend X for his victory over the	 comrade. Following this introduction, the two Reploids merge into their new form: Godkarmachine O Inari. In this new form, Byte becomes the lower half and the four legs, while Bit's body forms the upper half of the centaur, obtaining both Bit's swordsmanship and energy shots~\cite{wayback:X3_resources}, and Byte's strength and brutality~\cite{wiki:Godkarmachine}.

Due to their backstory, all the attacks Godkarmachine performs are callbacks of the his original forms. The first move he can employ is the \emph{Rocket Punch}. With this move, the boss first fires his left punch towards X to catch him and, if successful, rams X into the roof, then hangs X in midair and fires the right one to hit him into the wall. The second attack Godkarmachine can perform is the \emph{Shockwave}, an energy beam emitted from their saber which, upon contact with the wall, splits into two minor beams traveling upwards and downwards respectively.\begin{figure}[htp]
	\centering
	\begin{subfigure}{\linewidth}
		\centering
...
Fighting Godkarmachine fairly can be very challenging. The boss occupies most of the room, and all their attacks can be hard to avoid due to this limitation. Moreover, they deal heavy damage that can only be mitigated by the armor's shield, while Godkarmachine is only vulnerable in the upper half of their body, with the lower half being invincible. The best solution in this scenario is to have sharp reflexes, to dodge incoming attacks and hit using the boss's weakness, the Ray Splasher, which comes with the advantage of having a wide spread, meaning it can hit the boss even if X is not perfectly aligned with him. Trying to abuse the boss's immobility to hit from behind is not suggested, as his homing shots are far more powerful and will likely defeat X before he manages to kill the boss. This strategy, however, becomes viable if players decide to fight Godkarmachine exploiting a glitch in his fight. Using such glitch, described in section~\ref{glitch:Karmachine}, will paralyze the boss indefinitely, eliminating any difficulty from the fight. 
...
The second stage of Doppler's laboratory takes place on a higher floor, with mountains visible outside. The appearance and difficulty of the level depend on the outcome of X's clash with Vile mk-2. If X managed to defeat the secret boss using his main weakness, the stage remains intact when X visits it; however, if he didn't use the correct weapon or avoided the battle, Vile will still be alive, and X will find the stage ravaged, with different enemies and without a sub-boss, implying that Vile went on a rampage across the stage before X's arrival~\cite{wiki:Doppler_stage}.
...
\subsection{Vile mk-2}\label{boss:Vile2} 
\begin{figure}[htp]
	\centering
...
Doppler Stage C consists primary in the usual boss rematch. After an initial corridor, X will reach a room with eight teleporters, each one leading to a different rematch. In the center of the room players can find a special pillar which, upon hit, releases various pickups to restore X's health. However the pillar will break after receiving too much damages, but will regenerate every time a rematch has been completed.
...
In battle, Doppler himself is not a difficult boss, but the main gimmick of the fight can make him particularly challenging. Doppler's main attack pattern consists of two alternating moves: the \emph{Electric Shot}, a volley of three spheres of electricity fired at X, and the \emph{Charge}, where Doppler engulfs himself in fire and charges towards X. Doppler uses these attacks in sequence, starting with the electric shots followed by a charge. Additionally, Doppler can use the \emph{Attack Absorb} move, creating a barrier that heals him for double the damage he would have taken~\cite{wiki:X3_enemies}, extending the battle further if players attack at the wrong moment. However, Doppler only uses this move while X is shooting at him and he is standing still, so players can hit him safely during or immediately after an attack. In this scenario, using the beam saber and Arm Parts together is not advisable: while the beam itself almost guarantees hitting Doppler, cutting his health in half, the remaining slashes are absorbed by the barrier, nullifying the previous damage. Still, it is still possible to hit Doppler with the saber by understanding that the entire animation of X swinging the blade can deal damage, even at the very beginning. This allows the player to bait Doppler into using the charge, then positioning themselves near the boss while facing away and releasing the shot. If X is close enough, he will swing the saber away from Doppler, but the boss will still suffer damage due to X having a slim hitbox behind him when the swing animation begins. This allows to hit Doppler safely with the saber without fearing to accidentally heal him. The file \path{videos/X3/Doppler_2hit.mp4} shows how to perform such technique.
...
\end{figure}
Despite the simple attack pattern, attention is crucial when battling Sigma, particularly since this is only the first of two forms. Here, the primary challenge arises not from Sigma's attacks, which still inflicts considerable damages, but from the shield that protects him from X's attacks, leaving Sigma vulnerable only when he is attacking. The optimal strategy for this fight is to stick to the wall above him, as no attack, except for his shield, can harm X in that position, and use one of Sigma's primary weaknesses, the Spinning Blade, to safety hit him due to the boomerang-like behavior of the weapon, that allow the blade to turn backward and strike Sigma while he's shooting fireballs. Using Sigma's second weakness, the Frost Shield, is not recommended, as the slow-moving weapon requires X to be in front of Sigma, exposing him to all his attacks.
...
\end{figure}
Upon the destruction of his initial body by X, Sigma was forced to activate the ultimate battle body crafted for him by Doppler. Constructed using data from the most advanced reploids, Kaiser Sigma's formidable body boasted an array of powerful weapons, including guided missiles, remote-controlled mines, and a beam laser~\cite{wayback:X3_resources}. Despite being equipped with boosters to enhance mobility, the sheer size of Kaiser Sigma's body made these additions ineffective in confined spaces, preventing the body from reaching its intended high-speed capabilities. Still, despite the significant power boost, Sigma was once again defeated, and his new body destroyed. This forced Sigma to assume his viral form in an attempt to possess X, who was forced to flee. After a brief chase, Sigma cornered X, but before attempting to possess him, Doppler/Zero intervened, employing a vaccine that caused Sigma to fade away.

The battle against Kaiser Sigma is widely considered one of the most challenging Sigma fights in the entire series~\cite{wiki:Sigma3}. This difficulty arises from a combination of factors that, despite the boss's simple pattern, significantly increase the battle's complexity. A primary factor is Sigma's formidable damage output despite, in this form, Sigma can unleash only three attacks: \emph{Mines}, \emph{Missiles}, and \emph{Beam}. The first, \emph{Mines}, is probably the most annoying, as Sigma continuously releases floating mines from his shoulders in an alternating way, which either home on X or move along straight paths.\begin{figure}[htp]
	\centering
	\begin{subfigure}{.3\linewidth}
		\centering
...
\end{figure}
The second move, \emph{Missile}, involves Sigma firing a barrage of homing missiles from his left shoulder after hovering to the left portion of the arena, and can either release a volley of missiles all at once or one at the time in rapid sequence. The third attack, \emph{Beam}, shows Sigma firing a wide beam causing constant damage to X as long as he remains within it, ignoring invincibility frames, and adjusting the beam's direction upward or downward, depending on X's position. However, the beam can also destroy Sigma's own mines, ans Sigma will initiate this attack only after hovering from the left corner to the right one of the arena. 

While all of these attacks may appear avoidable, the crucial point is that Kaiser Sigma's massive body consistently occupies most of the entire arena, leaving limited space for avoiding them. As such, players must move in the confined space while avoiding either Sigma's beam or missiles and the mines simultaneously, often leading to quickly accumulate damages and an early conclusion to the fight. The second factor impacting the battle's difficulty lies in Kaiser Sigma's resistance to damage. This form is immune to all special weapons and buster attacks, with the sole exceptions being charged shots and the Z-saber. Adding to the challenge, despite Sigma's colossal body, his only vulnerable point is the head, but aiming directly at it is ineffective, as all shots will strike Sigma's chest first, nullifying the attack. To overcome this, players must aim \emph{above} Sigma's head, specifically at his right shoulder. Executing this technique is not trivial, as players must aim while sliding off a wall, while also avoiding Sigma's barrage of attacks.
...
Following Kaiser Sigma's defeat, Sigma manifests in his viral form, and players regain control of X for a brief escape sequence. Players must quickly climb the arena walls (the roof will vanish) while Sigma hinders their movements and lava begins to rise. Falling into the lava does not result in immediate death, inflicting only one point of damage. Nevertheless, it remains possible for X to be defeated, forcing to restart the entire boss fight. Upon reaching the end of the corridor, the final scene unfolds, leading to the game's credits.
