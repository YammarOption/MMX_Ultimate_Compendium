...
The couple then decides to split: Zero embarks on a mission to locate the enemy fortress, while X takes on the task of confronting Sigma's mavericks, gaining new powers and strength along the way. Eventually, he reunites with Zero, who had located Sigma's fortress flying over the sea. Upon reaching the fortress's entrance, they are immediately faced with Vile and his formidable ride armor and despite Zero courageously challenging Vile in a one-versus-one battle, they are ultimately captured. As Vile rejoices over his apparent triumph, Zero seizes an opportunity to break free from his confinement, attaching himself to the ride armor and detonating, destroying the armor. Witnessing Zero's sacrifice, X finds new determination, empowering him to break free as well. He confronts Vile, now vulnerable, and successfully defeats him in battle.
...
Following Zero's demise, X continues his mission and infiltrates Sigma's fortress and confronts its dangers, including re-facing all the previously defeated mavericks. Eventually, X arrives at Sigma's lair, where the villain awaits. Sigma initially displays arrogance, acting surprised that X managed to reach him and claiming he could easily destroy him, but ultimately decides to let his pet, Velguarder, deal with X as he watches the fight unfold. X proves his skill by successfully defeating Velguarder, slightly changing Sigma's perspective a bit, who now understands why Zero placed his trust in X. Acknowledging that X could have been an hunter as strong as he was, Sigma proceeds to face X himself, but at last is X whom emerges victorious, destroying all Sigma's body but his head. Sigma's head then combines with a giant wolf-based mechaniloid, providing him with a new body to continue the battle against X. Despite Sigma's tenacity, however, X manages to destroy the new body too, effectively putting an end to Sigma. 

Just as the fights end, the fortress begins to explode, while Sigma blames X for destroying his dream of a world exclusively for reploids. The fortress's imminent destruction forces X to teleport to safety outside. Contemplating the consequences of the destruction he has witnessed and the sacrifices made for victory, X questions if fighting was the right choice, wondering if an alternative path could have been taken. As the flying fortress sinks into the ocean, X realizes that he will face more battles before finding the answers he seeks. After that, the game's credits roll. Once the credits are over, however, a message from Sigma plays, revealing that his spirit is still intact, awaiting the construction of a new body to challenge X once again. 
\begin{figure}[htp]
	\centering
...
While Maverick Hunter X largely follows a similar plot to the original Mega Man X game, it does introduce some differences, particularly regarding the relationships between characters and X's background~\cite{wiki:MM_MHX}. One of the significant points of divergence pertains to X's backstory before the commencement of the war. Below is a summary of the differences between \textit{Maverick Hunter X} and Mega Man \x:
...
Despite these differences in character portrayal, Zero's story remains consistent between both games. He first appears at the end of the highway, rescuing X from Vile's grasp and requesting X to handle Sigma's forces while he searches for the enemy hideout. Later, Zero is spotted at the entrance of Sigma's fortress, acting as a diversion to enable X to infiltrate the fortress undetected. Finally, Zero is present during the final showdown with Vile, where he sacrifices himself to destroy Vile's armor, enabling X to defeat him. In his final moments, Zero speaks to X, urging him to go and confront Sigma, eventually even giving X his own Z-buster as a backup, in case X hasn't upgraded his own buster yet.
...
\item \textbf{Wall-jumping}: X gains the ability to jump onto walls to climb them, as well as slide down for a controlled descent. He can also dash-jump off walls to cover greater distances.
...
\item \textbf{Sub-Weapon} charging~[\ref{X1:sub_weapon}]: With the buster upgrade, X can not only increase the charge of his primary X-buster by one stage, but he can also charge his other sub-weapons to increase damage or alter their functionality.
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X1/weapons/Homig_T.jpg} Homing Torpedo}{Homing Torpedo}}\label{Homing_torpedo}
When X equips this sub-weapon, he gains the ability to fire up to two torpedoes (three in Maverick Hunter X)~\cite{wiki:Homing_torpedo}. These torpedoes possess the unique characteristic of tracking enemies. As they accelerate, they automatically target the nearest enemy and follow its movements. When charged, X can release a fan of four fish-shaped missiles (six in Maverick Hunter X). These charged missiles exhibit increased speed and attack power, making them more effective homing projectiles against enemies. Players obtain this weapon after defeating Launch Octopus~[\ref{boss:Launch_octopus}].
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X1/weapons/Rolling_S.jpg} Rolling Shield}{Rolling Shield}}\label{Rolling_shield}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X1/weapons/F_wave.jpg} Fire Wave}{Fire Wave}}\label{Fire_wave}
The Fire Wave transforms X's buster into a powerful flamethrower, capable of dealing continuous damage to enemies, but at the price of not being able to use it underwater.\begin{figure}[htp]
	\centering
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X1/weapons/E_Spark.jpg} Electric Spark}{Electric Spark}}\label{Electric_spark}
...
In the original Mega Man X, when this weapon is charged, X releases two electric columns in front and behind him, which move in their respective directions. On the other hand, in Maverick Hunter X, the weapon generates electricity in all directions starting from X's body, covering the entire screen. 
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X1/weapons/B_cutter.jpg} Boomerang Cutter}{Boomerang Cutter}}\label{Boomerang_cutter}
...
\subsection{\texorpdfstring{\includegraphics[width=12px, height=10px]{figures/X1/weapons/S_ice.jpg} Shotgun Ice}{Shotgun Ice}}\label{Shotgun_ice}
...
During the boss battle, Launch Octopus has three main techniques: the \emph{Homing Torpedo}, a charged version of the previous attack (which is faster and more powerful), and the \emph{Energy Drain}.  The former is often Octopus' way to start the fight, to switch then to one of his other two attacks. Notably, all version of his homing missiles can be destroyed by shooting at them. When using his Energy Drain attack, Octopus swims in the upper left or right corner, and spins to create a whirlpool to suck X in and drain his energy to replenish his own.
\begin{figure}[htp]
	\centering
...
\end{figure}However, due to the long permanence away from civilization, he grew tired of his mission in the South Pole and sought a way to escape. When Sigma's rebellion began, it provided Chill Penguin with the opportunity he needed to leave the South Pole and join Sigma's cause. In Mega Man X1, Chill Penguin fled  after killing his own commander, and took over a mountain base to cause chaos with avalanches~\cite{Xcoll1:Manual_X1}, while in \textit{Maverick Hunter X}, it was Sigma who recalled Chill Penguin from the South Pole and enlisted him in the 17th division~\cite{MHX:manual}, alongside a conspicuous payment to ensure his loyalty~\cite{wiki:MMX_script}. Penguin was known to be a belligerent and rowdy individual, sometimes showing jealousy towards Flame Mammoth due to the difference in size (\ref{boss:Flame_mammoth}).
...
Chill Penguin is often considered the easiest boss of the game and the first one a player should face due the simplicity of avoiding his attacks, also thanks to the Foot Parts the stage gives to the player. He has four main attacks which he performs at random~\cite{wiki:Chill_Penguin}. When using his \emph{Shotgun Ice}, Chill Penguin will shoot four frozen projectiles which travel in a straight line, although some of them  can fall off and slide on the ground, that shatter upon contact with a wall and that can nullify X's shots when the two meet. Other times instead of his projectiles he will emit his \emph{ice breath}, which will create two penguin-shaped \emph{ice statue} and freeze X if he is in the breath's range. The sculptures act both as obstacles that damage X and as covers since they blocks all projectiles, and require few shots to destroy them. 
...
Inside the tunnel, an earthquake triggers rocks to fall from the ceiling, potentially harming X on impact. Furthermore, enemies also drop from above, these being the  \hyperlink{enem:Crag_Man}{Crag Men} which mimic rocks descending onto X as he passes beneath, only to transform upon hitting the ground. This part becomes significantly less perilous if X has already defeated RT-55J (the source of the earthquake) or obtained the helmet parts (which render X immune to falling rocks). In either case, the Crag Men will consistently appear.
The final portion of the stage introduces a Ride Armor section. Early on, X can acquire a Ride Armor to navigate through this segment, which now features mud ponds that cause X and the Armor to sink slowly, alternated with small platforms where the player can stand. While X can free himself by jumping repeatedly, once the Ride Armor fully sinks, it's lost. This can become a problem since enemies in Ride Armors will start appearing near the end of the stage, and defeating them without a Ride Armor can be a hard task. Ultimately, at the conclusion of this section, players find the boss door.
...
\end{figure}
The armor part is located close to the beginning of the stage, before entering the tunnel. If X climbs the wall above the mentioned tunnel (achieving this by executing a dash-jump from above the pit) and then proceeds to the right, he will arrive at an arena. Upon his entrance, the exit to this arena will promptly seal shut due to falling rocks,  followed by the appearance of the RT-55J miniboss. The primary attack of this miniboss involves utilizing its claw to grasp X (or to pull itself toward a wall if the claw connects with it). Alternatively, if X is beyond the range of the claw, RT-55J will leap onto him. The miniboss' vulnerable points are its head and its entire body but the latter is often covered by the claw, which is immune to all forms of attack. Consequently, the head becomes the primary target to strike. Given the absence of invincibility frames, the Storm Tornado proves to be the most effective choice, although the Boomerang Cutter is often considered its main weakness~\cite{wiki:RT55J}. As the battle progresses, RT-55J will begin emitting smoke, indicating that its health is low. Once the miniboss is defeated, the Light Capsule containing the enhancement (the Armor parts in the original and Buster parts in the remake) will ascend from the ground.
...
\begin{figure}[htp]
	...
	\includegraphics[height=3.5cm]{figures/X1/Sting_chameleon/MHXStingChameleon.jpg}
...
\end{figure} 
Confronting Sting Chameleon without the appropriate weapon can be challenging; however, this dynamic shifts when employing the Boomerang Cutter, his vulnerability. While initially it seems this weapon only inflicts increased damage, what really this weapon causes is to loop his AI into a fixed pattern that can be exploited to quickly end the battle. Upon being struck, in fact, Chameleon consistently moves to the opposite side of the room to execute his Thorn Rain attack. This allows the player to remain at the room's center, deploying Boomerang Cutters where Chameleon will reappear and, upon impact, the player can pivot and repeat the process in the opposite direction. Notably, the player need not be concerned about weapon energy, as missed boomerangs will return and replenish the utilized energy (refer to the video \path{videos/X1/Sting_Chameleon_boomerang_loop.mp4} for a visual example).

According to records, Sting Chameleon stood at a height of 177 cm and weighed 77 kg, earning him the title of \textit{``Frightening Forest's Strike''}~\cite{book:MMX_Complete_art}. After defeating Sting Chameleon, X will acquire the Chameleon Sting (as detailed in section~\ref{Chameleon_sting}) weapon to augment his arsenal.
...
Vile possesses three main attacks~\cite{wiki:Vile}. His first move is the \emph{Electroshock Bullets}, the same projectile employed to immobilize X in prior encounters. If this attack connects, it paralyzes X briefly, leaving him vulnerable to follow ups. The second attack is the \emph{Head Bombs}: while airborne, Vile releases two knee-launched bombs that, upon impact, disintegrate into two sparks that travel along the ground in opposite directions. Lastly, Vile can dash at X with notable speed, dealing contact damage. This attack can be executed by dashing along the ground or leaping into the air before landing on X.

The true challenge of this battle relies not solely from Vile's attacks but rather his agility. Vile consistently traverses the screen, even going outside the visible area. Coupled with his relatively small hitbox, this makes targeting him with regular shots challenging. This changes a little when using Vile's main weakness, the Rolling Shield, as it can travel the ground and hit him while he's dashing across. However, Vile is also considerably vulnerable to another weapon, the Homing Torpedo. Utilizing this sub-weapon eases the battle, as it automatically homes in on Vile's location, allowing the player to concentrate on dodging incoming attacks.
...
The fight with Bospider is significantly influenced by RNG (Random Number Generator). Inside the boss chamber are four rails, connected by randomly generated horizontal lines. Bospider emerges atop one of the main rails unpredictably and descends according to the principles of the ``Amida Lotter'' (Amidakuji), altering the rail it descent on each time a horizontal line is encountered. Once on the ground, Bospider briefly exposes its crimson core, vulnerable to attack, before  ascending and restarting its attack sequence. Additionally, Bospider periodically spawns four Petitpiders (whether from the top or midway during descent), which traverse surfaces and scale walls until reaching the ceiling~\cite{wiki:Bospider}.
...
To defeat Bospider, the player must predict its landing position and strike it before the core retreats (note that only charged shots and Shotgun Ice inflict notable damage). This is challenging process, particularly in the latter phase of the battle, during which Bospider increase its attack speed. 
...
Subsequently, a long corridor unfolds with more enemies, including Ride Armors. Luckily, immediately at the beginning of the section, an Armor Soldier is stationed beside a free Ride Armor. If the player is quick enough, it is possible to eliminate the soldier before they claim the armor, allowing to seize it for the upcoming section. Failing this will cause the enemy to claim the armor, and players are suggested to slightly backtrack to allow the Armor Soldier and the Ride Armor to respawn, providing another opportunity to acquire the armor. Near the end of this segment, players arrive at  a vertical passage, divided into two paths by a wall: the left path involves X scaling moving platforms via wall jumps while evading cannons, while the right path involves ascending ladders as cannons traverse the walls. regardless of the chosen route, upon reaching the summit, X faces a rematch against Storm Eagle~[\ref{boss:Storm_Eagle}]. After this confrontation, by leaping the gap to the right of the arena, players arrive at the final segment of the stage.

This concluding portion involves yet another climb. Here, enemies positioned on higher platforms attack X during the climb. Upon reaching the summit, the player encounters the last boss door of the stage.
...
\end{figure}
Rangda Bangda was originally designed as a mechanism to crush trespassers entering Sigma's hideout. However, due to a miscalculation, its walls only close halfway, forcing its creators to reconfigure its design by adding movement and offensive capabilities the eyes and the nose. The inspiration for its creations came from a mural that Sigma encountered in Southeast Asia, during a mission as a Maverick Hunter, which pleased him~\cite{wayback:X_resources}.

Rangda Bangda showcases four distinct attacks, three of which starts from its eyes~\cite{wiki:Rangda_bangda}: the green eye shoots projectiles toward X; the blue eyes home to X's location in straight line, making it easily avoidable by slightly moving to the left or right; the red eye combines the characteristics of the blue and green counterparts, either releasing projectiles while advancing toward X or discharging three projectiles simultaneously. For its final attack, Rangda Bangda seals the walls, leaving only the spike pit exposed, as its nose starts to bounce between the walls (the initial movement is consistently diagonal upwards~\cite{stratwiki:Sigma_stage_2}). After a while, the nose halts its movement, and the walls reopen.
...
Fighting this boss is relatively manageable, but it shouldn't be underestimated due to its capacity for inflicting significant damage, and a single mistake could result in falling onto the spikes and restarting the fight. To win, players must destroy all its components—the two eyes and the nose—through attacks. The Chameleon Sting, its weakness, is particularly effective as it targets diagonally, allowing to easily strike the eyes in various positions and the nose. However, it's suggested to reserve the nose for last, as destroying it closes the walls permanently, making  it challenging to identify the opened eye and its location, and making it more difficult to target.
...
This stage stands as the ultimate challenge preceding the final boss encounter. It houses all the remaining rematches that were omitted from prior stages, separated by brief sections featuring enemies and restorative items. Notably, this stage boasts the highest concentration of rematches within the entire game, with five out of the eight available.

The level starts at the base of a vertical chamber, necessitating X to climb up to reach the initial boss encounter: Armored Armadillo~(\ref{boss:Armored_Armadillo}). Here a glitch can be exploited, allowing players to access the boss room without triggering the boss's appearance, effectively skipping the battle entirely (refer to~section~\ref{Armadillo_skip} for more details). Subsequently, the next rematch is against Sting Chameleon~(\ref{boss:Sting_chameleon}), followed by a descending path leading to Spark Mandrill~(\ref{boss:Spark_mandrill}). A brief underwater segment paves the way for the rematch against Launch Octopus~(\ref{boss:Launch_octopus}), succeeded by an long corridor culminating in the final rematch against the last remaining boss: Flame Mammoth~(\ref{boss:Flame_mammoth}).
Upon winning against this last boss, players can move forward and face the stage's final boss.
...
\subsection{D-REX}\label{boss:D-Rex}
\begin{figure}[htp]
	\centering
	\includegraphics[height=\portraitsize]{figures/X1/Sigma_stages/Drex.jpg}
	\caption{D-Rex's design(\cite{book:MMX_Complete_art})}
\end{figure}
D-REX serves as the last boss in the third stage of Sigma's Hideout. Originally ideated as a colossal 30-meter tall weapon of destruction, it draws inspiration from carnivorous dinosaurs of the past. However, the discovery of Sigma's fortress put a premature halt to its construction, leading to the hasty assembly of its sole completed component, the head, onto a mobile tank. Notably, the two components, the upper and lower jaws, operate independently and are capable of autonomous movement. D-REX exploit this unique attribute to execute a scissor-like attack, mercilessly crushing adversaries. Furthermore, each component possesses an individual power reactor, capable of absorbing energy, which can be aggregated into a formidable energy sphere~\cite{wayback:X_resources}.

Engaging D-REX requires a series of quick movements. The boss primarily resorts to slamming one of its segments into the wall (\emph{Tackle}), targeting X in the process. The lower component is employed when X is grounded, while the upper section is used when X is airborne. If both components hist the a wall simultaneously, X will remain stunned and fall from the wall. A viable way to avoid such attacks requires standing on the ground and executing a dash-jump over the lower segment as it draws near. It's worth noting, however, that standing on the lower segment triggers the upper part to fall, thereby crushing X for massive damage (\emph{Sandwich} attack). Furthermore, D-REX will occasionally charge its \emph{Electric shot} attack, manifesting as a substantial energy sphere that is subsequently launched toward X. 
\begin{figure}[htp]
	\centering
...
Fighting effectively against defeat D-REX requires precision targeting in hitting exclusively its upper segment, since the lower one is immune to damages. This task can become challenging, due the difficulty of aiming while dodging incoming attacks, and especially since the upper segment often remains airborne. Luckily, the boss' primary weakness—the Boomerang Cutter—not only inflicts substantial damage but also overcomes the aiming difficulty, as its upward trajectory is triggered when on ground, allowing accurate strikes and without hindering mobility.
...
\end{figure} 
Velguarder's role as a boss demands caution due to his incredible aggressiveness, especially given his high-speed attacks that can prove hard to avoid. The inclusion of wall-jumping abilities further complicates things, as traditional wall jumping is not a safe option for the player anymore. Additionally, Velguarder can also attack from distance, deploying two distinct types of projectiles: \emph{plasma shots}, dispatched in a curved trajectory at varying heights, and a \emph{fire breath}, which starts with a downward trajectory before curving upwards towards X's position.

Notably, Velguarder's weak point is the Shotgun Ice, which inflicts considerable damage and its ricochet properties also guarantee shots to hit even while Velguarder is airborne. Alternatively, an effective strategy involves employing the Hadoken technique to eliminate Velguarder quickly at the very beginning of the battle. This approach capitalizes on Velguarder's predictable initial sequence: a diagonal-upward leap towards the wall behind X, followed by a diagonal-downward descent to return to the starting point and a dash towards X. Despite the speed of this pattern, the brief time used by Velguarder gives player sufficient time to execute the Hadoken input, quickly ending the fight (refer to the first segment of \path{videos/X1/Velguarder+Sigma_hadoken.mp4} for optimal timing).
...
\end{figure}
Sigma's motives for rebellion maintain consistency across both the original game and its remake, the latter also helped by the "Day of $\Sigma$" OVA, which unveils the narrative preceding Sigma's treason. Across these versions, it becomes evident that Sigma's fascination with reploid potential—exemplified by X—pushed him to believe that reploids were to overtake humanity in the evolutionary hierarchy, making humans an halt to the process. With newfound resolve, Sigma took it upon himself to speed up this process through the annihilation of humanity, a task he sees as an act of evolution\footnote{\enquote{You see, evolution requires sacrifice} - Sigma - Day of $\Sigma$~\cite{wiki:Day_of_Sigma_Script}}, his conviction to create a world for reploid\footnote{\enquote{We would have begun $\dots$ the age $\dots$ of $\dots$ Reploids$\dots$} - Sigma - Mega Man X~\cite{wiki:MMX_script}} only held steadfast, and those opposing this path were swiftly labeled traitors\footnote{\enquote{After all, I’ve entrusted all dealings with traitors to him (Velguarder A/N)} - Sigma - Mega Man X (Japanese script)~\cite{wordpress:X_japanese_script}}. The fact of being the strongest reploid, which Sigma is fully aware of, enables him to observe other reploids from a vantage point. This however leads him to underestimate potential threats, exemplified by his initial presumption of X's capabilities. Nevertheless, Sigma boasts an indomitable combat spirit, invigorated when he recognizes X as a worthy adversary—even if overshadowed by his inherent hubris. Yet, Sigma's tactics extend beyond sheer power, including strategic command cultivated during his times as a Hunter leader. He effectively deploys troops in tactical locations—mines, power plants, and factories—to exert control, simultaneously collecting armaments and resources to secure an upper hand against opponents. Furthermore, Sigma prepares contingency plans for his defeat, exemplified by his final message within the Japanese version of Mega Man X, hinting at his creation of new bodies to house his essence, viewing his defeat as merely a temporary setback (he explicitly speaks of the defeated Sigma as \textit{``not my true self''} and as \textit{``more like another body''}~\cite{wordpress:X_japanese_script}.).

Mirroring Velguarder's approach, Sigma capitalizes on rapid movements to barrage the player during the battle. His most used attack is the \emph{Triangle Kick}—quick jumps between walls to converge on X's position. However, Sigma's larger hitbox makes dodging it more challenging than the Velguarder encounter. Sigma also employs a ranged attack, the \emph{Machine Gun}, which fires a volley of laser bullets he shoots from his forehead. This attack is relatively easy to avoid, as Sigma fires aiming to the ground, and never up. Finally, Sigma will also occasionally swing his \emph{Beam Saber} to deal damage, but this attack is rare as Sigma will perform it only if X is at very close range.
\begin{table}[htp]
	\centering  
	\begin{tabular}[h]{l c}
		\toprule
...
Exploiting Sigma's vulnerability to the Electric Spark significantly eases the battle, as its projectiles move along the walls thus creating a trap for Sigma, as he will keep jumping between walls, thus getting hit by a traveling spark almost every time. Two strategies are effective in combating Sigma with minimal damage. The first exploits his AI behavior to induce an endless loop of wall-jumping, resulting in dealing constant damage to him. This strategy requires  X to initially climb up the left wall, starting Sigma's attack sequence, followed by dropping down and firing shots as Sigma chases (preferably while Sigma is jumping from the left wall to the right one). Repeating this pattern indefinitely entraps Sigma in a perpetual pursuit, while getting hit by the sparks and keeping the player relatively safe. Alternatively, players can quickly execute the Hadoken as the battle begins, defeating Sigma instantly. However, the window for input is notably short, as Sigma promptly lunges toward X in his initial attack sequence (refer also to \path{videos/X1/Velguarde+Sigma_hadoken.mp4}).
...
\end{figure}
Even in its unfinished state, this form proves to be a tough adversary for X in terms of combat prowess and defensive capabilities. Wolf Sigma delivers formidable damage through its array of attacks while remaining immune to the majority of X's arsenal, including the hadoken. Furthermore, the sole vulnerable point for X to target lies within Sigma's head, placed high in midair at the center of the arena. X's sole means to reach the necessary height for attacking is to employ the mobile hands as platforms, while the only weapons with the capacity to damage him are the Rolling Shield and the Spiral Crush Buster.

Wolf Sigma possesses a diverse repertoire of attacks, all capable to deal substantial damage to the player, even if equipped with the Body Part. During the confrontation, Sigma's hands follow an orbital path near the corners of the arena. When one of them stops and opens up of one hand, this indicates that it will fly towards X, regardless of his current position.  This situation is an optimal opportunity for X to leap onto it, as the hand momentarily halts upon reaching X's vicinity before returning to its position. To complements this, Wolf Simga possesses three distinct attacks in his arsenal.\begin{figure}[htp]
	\centering
	\begin{subfigure}{\linewidth}
		\centering
...
\end{figure} His \emph{flamethrower} can cover the central arena with fire, his \emph{Consecutive Bullets} attack fires a barrage of projectiles downward, and his \emph{Hand Electric Shocks} emanates lightnings from his hands at regular intervals. This latter attack should functions as a deterrent, to counter players who might try and stand on the hands to stall time, but it can be easily avoided as the electricity  shoots vertically from the hand's center, meaning that if X remains near the edge he will avoid the attack. \begin{figure}[htp]
	\centering
...
The best strategy to deal with this boss  revolves around X positioning himself atop one of the hands, taking advantages of the intervals when  Sigma's head is in reach of a shot. However, careful movements are essential to avoid falling from the platform, which would make X vulnerable to Sigma's attacks. Another important point to keep in mind is that, occasionally, the opposing hand may launch its attack in a horizontal trajectory, effectively pushing X off the hand.


Upon defeating his final form, Sigma collapses and the fortress starts exploding. X is transported outside the fortress, and the game plays the concluding scene before the starting the credits.
...
In this chapter a series of information are collected regarding hidden game mechanics as well as useful tips and even some glitches known in the game.
